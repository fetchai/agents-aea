The `fetchai/p2p_libp2p:0.23.0` connection allows AEAs to create a peer-to-peer communication network. In particular, the connection creates an overlay network which maps agents' public keys to IP addresses.
...
``` bash
aea run --connections fetchai/p2p_libp2p:0.23.0
```
...
To learn more about how to configure your `fetchai/p2p_libp2p:0.23.0` connection consult the `README.md` file supplied with the connection package.
