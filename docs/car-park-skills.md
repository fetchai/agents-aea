...
``` bash
aea fetch fetchai/car_detector:0.25.0
cd car_detector
aea install
aea build
```
...
aea add skill fetchai/carpark_detection:0.22.0
...
``` bash
aea fetch fetchai/car_data_buyer:0.26.0
cd car_data_buyer
aea install
aea build
```
...
aea add connection fetchai/soef:0.20.0
aea add connection fetchai/ledger:0.16.0
aea add skill fetchai/carpark_client:0.23.0
aea config set --type dict agent.dependencies \
'{
  "aea-ledger-fetchai": {"version": "<2.0.0,>=1.0.0rc3"}
}'
