...
``` bash
aea fetch fetchai/simple_service_registration:0.32.0 && cd simple_service_registration && aea install && aea build
```

This AEA will simply register a location service on the <a href="../simple-oef">SOEF search node</a> so we can search for it.

We first create the private key for the service provider AEA based on the network you want to transact. To generate and add a private-public key pair for Fetch.ai `Capricorn` use:
``` bash
...
First, create the private key for the search AEA based on the network you want to transact. To generate and add a private-public key pair for Fetch.ai `Capricorn` use:
``` bash
aea generate-key fetchai
aea add-key fetchai fetchai_private_key.txt
```
