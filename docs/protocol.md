...
## `fetchai/default:1.1.3` protocol

The `fetchai/default:1.1.3` protocol is meant to be implemented by every AEA. It serves AEA to AEA interaction and includes three message performatives:
...
Each AEA's `fetchai/error:0.18.3` skill utilises the `fetchai/default:1.0.0` protocol for error handling.

## `fetchai/oef_search:1.1.3` protocol

The `fetchai/oef_search:1.1.3` protocol is used by AEAs to interact with an <a href="../simple-oef">SOEF search node</a> to register and unregister their own services and search for services registered by other agents.

The `fetchai/oef_search:1.1.3` protocol definition includes an `OefSearchMessage` with the following message types:
...
The `fetchai/fipa:1.1.3` protocol definition includes a `FipaMessage` with the following performatives:
...
The `fetchai/fipa:1.1.3` protocol also defines a `FipaDialogue` class which specifies the valid reply structure and provides other helper methods to maintain dialogues.

For examples of the usage of the `fetchai/fipa:1.1.3` protocol check out the <a href="../generic-skills-step-by-step" target="_blank"> generic skills step by step guide</a>.
