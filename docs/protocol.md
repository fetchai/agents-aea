...
## `fetchai/default:1.1.4` protocol

The `fetchai/default:1.1.4` protocol is meant to be implemented by every AEA. It serves AEA to AEA interaction and includes three message performatives:
...
## `fetchai/oef_search:1.1.4` protocol

The `fetchai/oef_search:1.1.4` protocol is used by AEAs to interact with an <a href="../simple-oef">SOEF search node</a> to register and unregister their own services and search for services registered by other agents.
...
For examples of the usage of the `fetchai/fipa:1.1.4` protocol check out the <a href="../generic-skills-step-by-step" target="_blank"> generic skills step by step guide</a>.
