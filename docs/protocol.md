...
## `fetchai/default:0.15.0` protocol

The `fetchai/default:0.15.0` protocol is meant to be implemented by every AEA. It serves AEA to AEA interaction and includes three message performatives:
...
Each AEA's `fetchai/error:0.15.0` skill utilises the `fetchai/default:0.15.0` protocol for error handling.

## `fetchai/oef_search:0.16.0` protocol

The `fetchai/oef_search:0.16.0` protocol is used by AEAs to interact with an <a href="../simple-oef">SOEF search node</a> to register and unregister their own services and search for services registered by other agents.

The `fetchai/oef_search:0.16.0` protocol definition includes an `OefSearchMessage` with the following message types:
...
The `fetchai/fipa:0.16.0` protocol definition includes a `FipaMessage` with the following performatives:
...
The `fetchai/fipa:0.16.0` protocol also defines a `FipaDialogue` class which specifies the valid reply structure and provides other helper methods to maintain dialogues.

For examples of the usage of the `fetchai/fipa:0.16.0` protocol check out the <a href="../generic-skills-step-by-step" target="_blank"> generic skills step by step guide</a>.
