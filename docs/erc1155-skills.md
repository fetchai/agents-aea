...
``` bash
aea fetch fetchai/erc1155_deployer:0.32.0
cd erc1155_deployer
aea install
aea build
```
...
aea add skill fetchai/erc1155_deploy:0.29.0
...
``` bash
aea fetch fetchai/erc1155_client:0.32.0
cd erc1155_client
aea install
aea build
```
...
aea add connection fetchai/soef:0.25.0
aea add connection fetchai/ledger:0.18.0
aea add skill fetchai/erc1155_client:0.27.0
aea config set --type dict agent.dependencies \
'{
  "aea-ledger-fetchai": {"version": "<2.0.0,>=1.0.0"},
  "aea-ledger-ethereum": {"version": "<2.0.0,>=1.0.0"},
...
Alternatively, use `aea get-multiaddress fetchai -c -i fetchai/p2p_libp2p:0.24.0 -u public_uri` to retrieve the address. The output will be something like `/dns4/127.0.0.1/tcp/9000/p2p/16Uiu2HAm2JPsUX1Su59YVDXJQizYkNSe8JCusqRpLeeTbvY76fE5`.
