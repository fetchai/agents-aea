...
``` bash
aea fetch fetchai/erc1155_deployer:0.28.0
cd erc1155_deployer
aea install
aea build
```
...
``` bash
aea fetch fetchai/erc1155_client:0.28.0
cd erc1155_client
aea install
aea build
```
...
Alternatively, use `aea get-multiaddress fetchai -c -i fetchai/p2p_libp2p:0.20.0 -u public_uri` to retrieve the address. The output will be something like `/dns4/127.0.0.1/tcp/9000/p2p/16Uiu2HAm2JPsUX1Su59YVDXJQizYkNSe8JCusqRpLeeTbvY76fE5`.
