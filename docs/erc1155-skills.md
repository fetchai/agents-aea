...
``` bash
aea fetch fetchai/erc1155_deployer:0.34.2
cd erc1155_deployer
aea install
aea build
```
...
aea add skill fetchai/erc1155_deploy:0.31.3
...
``` bash
aea fetch fetchai/erc1155_client:0.34.2
cd erc1155_client
aea install
aea build
```
...
aea add connection fetchai/soef:0.27.3
aea add connection fetchai/ledger:0.21.2
aea add skill fetchai/erc1155_client:0.29.3
aea config set --type dict agent.dependencies \
'{
  "aea-ledger-fetchai": {"version": "<2.0.0,>=1.0.0"},
  "aea-ledger-ethereum": {"version": "<2.0.0,>=1.0.0"},
...
Alternatively, use `aea get-multiaddress fetchai -c -i fetchai/p2p_libp2p:0.27.2 -u public_uri` to retrieve the address. The output will be something like `/dns4/127.0.0.1/tcp/9000/p2p/16Uiu2HAm2JPsUX1Su59YVDXJQizYkNSe8JCusqRpLeeTbvY76fE5`.
