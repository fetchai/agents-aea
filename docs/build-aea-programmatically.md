...
## Initialise the AEA
We use the <a href="../api/aea_builder#aeabuilder-objects">`AEABuilder`</a> to readily build an AEA. By default, the `AEABuilder` adds the `fetchai/default:1.1.1`, `fetchai/state_update:1.1.2` and `fetchai/signing:1.1.1` protocols.
``` python
