...
## Initialise the AEA
We use the <a href="../api/aea_builder#aeabuilder-objects">`AEABuilder`</a> to readily build an AEA. By default, the `AEABuilder` adds the `fetchai/default:0.11.0` protocol, the `fetchai/stub:0.14.0` connection and the `fetchai/error:0.11.0` skill.
``` python
...
``` python
        ...
        message_text = b"my_aea,other_agent,fetchai/default:0.11.0,\x12\x10\x08\x01\x12\x011*\t*\x07\n\x05hello,"
