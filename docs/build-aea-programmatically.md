...
## Initialise the AEA
We use the <a href="../api/aea_builder#aeabuilder-objects">`AEABuilder`</a> to readily build an AEA. By default, the `AEABuilder` adds the `fetchai/default:0.15.0`, `fetchai/state_update:0.13.0` and `fetchai/signing:0.13.0` protocols.
``` python
...
If you now run this python script file, you should see this output:
    ...
    input message: my_aea,other_agent,fetchai/default:0.15.0,\x12\x10\x08\x01\x12\x011*\t*\x07\n\x05hello,
    output message: other_agent,my_aea,fetchai/default:0.15.0,...\x05hello
