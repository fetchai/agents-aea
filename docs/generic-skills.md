...
``` bash
aea fetch fetchai/generic_seller:0.23.0 --alias my_seller_aea
cd my_seller_aea
aea install
aea build
```
...
aea add skill fetchai/generic_seller:0.24.0
...
``` bash
aea fetch fetchai/generic_buyer:0.24.0 --alias my_buyer_aea
cd my_buyer_aea
aea install
aea build
```
...
aea add connection fetchai/soef:0.21.0
aea add connection fetchai/ledger:0.17.0
aea add skill fetchai/generic_buyer:0.24.0
aea config set --type dict agent.dependencies \
'{
  "aea-ledger-fetchai": {"version": "<2.0.0,>=1.0.0rc3"}
}'
