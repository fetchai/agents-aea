...
``` bash
aea fetch fetchai/thermometer_aea:0.24.0 --alias my_thermometer_aea
...
aea add skill fetchai/thermometer:0.23.0
...
``` bash
aea fetch fetchai/thermometer_client:0.25.0 --alias my_thermometer_client
...
aea add connection fetchai/soef:0.21.0
aea add connection fetchai/ledger:0.17.0
aea add skill fetchai/thermometer_client:0.23.0
aea config set --type dict agent.dependencies \
'{
  "aea-ledger-fetchai": {"version": "<2.0.0,>=1.0.0"}
}'
...
``` bash
aea eject skill fetchai/thermometer:0.23.0
```
...
Once you see a message of the form `To join its network use multiaddr 'SOME_ADDRESS'` take note of this address. (Alternatively, use `aea get-multiaddress fetchai -c -i fetchai/p2p_libp2p:0.20.0 -u public_uri` to retrieve the address.)
