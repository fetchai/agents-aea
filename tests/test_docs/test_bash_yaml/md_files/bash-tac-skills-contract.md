``` bash
aea fetch fetchai/tac_controller_contract:0.28.0
...
aea add skill fetchai/tac_control_contract:0.24.0
...
```
``` bash
aea fetch fetchai/tac_participant_contract:0.18.0 --alias tac_participant_one
...
aea add-key ethereum ethereum_private_key.txt
cd ..
aea fetch fetchai/tac_participant_contract:0.18.0 --alias tac_participant_two
...
aea add skill fetchai/tac_negotiation:0.26.0
aea config set --type dict agent.dependencies \
'{
  "aea-ledger-fetchai": {"version": "<2.0.0,>=1.0.0"},
  "aea-ledger-ethereum": {"version": "<2.0.0,>=1.0.0"}
