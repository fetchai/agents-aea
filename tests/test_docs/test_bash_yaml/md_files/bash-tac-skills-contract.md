``` bash
aea fetch fetchai/tac_controller_contract:0.32.2
...
aea add skill fetchai/tac_control_contract:0.27.3
...
```
``` bash
aea fetch fetchai/tac_participant_contract:0.22.2 --alias tac_participant_one
cd tac_participant_one
aea install
aea build
cd ..
aea fetch fetchai/tac_participant_contract:0.22.2 --alias tac_participant_two
...
aea add connection fetchai/soef:0.27.3
aea add connection fetchai/ledger:0.21.2
aea add skill fetchai/tac_participation:0.25.3
aea add skill fetchai/tac_negotiation:0.29.3
aea config set --type dict agent.dependencies \
'{
  "aea-ledger-fetchai": {"version": "<2.0.0,>=1.0.0"},
  "aea-ledger-ethereum": {"version": "<2.0.0,>=1.0.0"}
...
```
``` bash
aea get-multiaddress fetchai -c -i fetchai/p2p_libp2p:0.27.2 -u public_uri
```
``` bash
