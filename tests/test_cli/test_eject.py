...
class TestEjectCommands(AEATestCaseMany):
    ...
    def test_eject_commands_positive(self):
        ...
        self.add_item("connection", "fetchai/gym:0.8.0")
        self.add_item("skill", "fetchai/gym:0.8.0")
        self.add_item("contract", "fetchai/erc1155:0.10.0")

        self.run_cli_command("eject", "connection", "fetchai/gym:0.8.0", cwd=cwd)
        ...
        self.run_cli_command("eject", "protocol", "fetchai/gym:0.6.0", cwd=cwd)
        assert "gym" not in os.listdir(
            (os.path.join(cwd, "vendor", "fetchai", "protocols"))
        )
        assert "gym" in os.listdir((os.path.join(cwd, "protocols")))

        self.run_cli_command("eject", "skill", "fetchai/gym:0.8.0", cwd=cwd)
        assert "gym" not in os.listdir(
            (os.path.join(cwd, "vendor", "fetchai", "skills"))
        )
        assert "gym" in os.listdir((os.path.join(cwd, "skills")))

        self.run_cli_command("eject", "contract", "fetchai/erc1155:0.10.0", cwd=cwd)
