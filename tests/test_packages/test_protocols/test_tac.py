...
def test_tac_message_instantiation():
    ...
    assert TACMessage(type=TACMessage.Type.REGISTER, agent_name="some_name")
...
def test_tac_serialization():
    ...
    msg = TACMessage(type=TACMessage.Type.REGISTER, agent_name="some_name")
    ...
    with pytest.raises(ValueError, match="Type not recognized."):
        with mock.patch(
            "packages.fetchai.protocols.tac.message.TACMessage.Type"
        ) as mocked_type:
