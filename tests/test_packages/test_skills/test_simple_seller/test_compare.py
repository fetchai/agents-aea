...
#
# ------------------------------------------------------------------------------
"""This module ensures that the "simple_seller" skill under tests and packages have the expected changes and are otherwise identical."""
...
def test_compare_test_skill_with_actual_skill():
    ...
    # compare strategy.py
    strategy_file_original = Path(path_to_actual_skill, "strategy.py")
    strategy_file_test = Path(path_to_test_skill, "strategy.py")
    is_matched, msg = match_files(strategy_file_original, strategy_file_test)
    assert is_matched, msg
    ...
    assert msg == "      shared_state_key: null\n"
