...
from packages.fetchai.skills.tac_negotiation.behaviours import GoodsRegisterAndSearchBehaviour, TransactionCleanUpBehaviour
from packages.fetchai.skills.tac_negotiation.dialogues import OefSearchDialogue, OefSearchDialogues
...
class TestSkillBehaviour(BaseSkillTestCase):
    ...
    def setup(cls):
        ...
        cls.sender = cls._skill.skill_context.agent_address + "_" + str(cls._skill.skill_context.skill_id)
    ...
    def test_register_service(self):
        ...
        # _register_service
        mock_logger.assert_any_call(
            logging.DEBUG, f"updating service directory as {self.strategy.registering_as}."
        )
        has_attributes, error_str = self.message_has_attributes(
            actual_message=self.get_message_from_outbox(),
            message_type=OefSearchMessage,
    ...
    def test_search_services(self):
        ...
        for search in searching_for_types:
            ...
            mock_logger.assert_any_call(
                logging.INFO, f"searching for {search[1]}, search_id={message.dialogue_reference}."
            )
    ...
    def test_teardown_i(self):
        ...
        assert has_attributes, error_str
        mock_logger.assert_any_call(
            logging.DEBUG, f"unregistering from service directory as {self.strategy.registering_as}."
        )
...
class TestTransactionCleanUpBehaviour(BaseSkillTestCase):
    ...
    def setup(cls):
        """Setup the test class."""
        super().setup()
        cls.clean_up_behaviour = cast(TransactionCleanUpBehaviour, cls._skill.skill_context.behaviours.clean_up)
    ...
    def test_act(self):
        ...
        # operation
        with patch.object(self.transactions, "update_confirmed_transactions") as mocked_update:
            with patch.object(self.transactions, "cleanup_pending_transactions") as mocked_cleanup:
