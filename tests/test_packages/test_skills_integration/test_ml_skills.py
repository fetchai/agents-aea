...
class TestMLSkills(AEATestCaseManyFlaky):
    ...
    def test_ml_skills(self, pytestconfig):
        ...
        self.add_item("skill", "fetchai/ml_data_provider:0.22.0")
        setting_path = (
        ...
        self.set_config("agent.default_connection", "fetchai/p2p_libp2p:0.19.0")
        self.add_item("connection", "fetchai/ledger:0.16.0")
        self.add_item("skill", "fetchai/ml_train:0.24.0")
        setting_path = (
...
class TestMLSkillsFetchaiLedger(AEATestCaseManyFlaky):
    ...
    def test_ml_skills(self, pytestconfig):
        ...
        diff = self.difference_to_fetched_agent(
            "fetchai/ml_data_provider:0.25.0", data_provider_aea_name
        )
        assert (
            diff == []
        ...
        diff = self.difference_to_fetched_agent(
            "fetchai/ml_model_trainer:0.26.0", model_trainer_aea_name
        )
        assert (
            diff == []
        ), "Difference between created and fetched project for files={}".format(diff)
