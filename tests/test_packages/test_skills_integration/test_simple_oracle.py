...
class TestOracleSkills(AEATestCaseManyFlaky, UseGanache):
    ...
    def test_oracle(self, erc20_contract, oracle_contract):
        ...
        default_routing = {
            ...
            "fetchai/contract_api:0.14.0": "fetchai/ledger:0.17.0",
            "fetchai/http:0.15.0": "fetchai/http_client:0.21.0",
            "fetchai/prometheus:0.6.0": "fetchai/prometheus:0.6.0",
        }

        # add packages for oracle agent
        self.set_agent_context(oracle_agent_name)
        self.add_item("connection", "fetchai/p2p_libp2p:0.20.0")
        self.add_item("connection", "fetchai/ledger:0.17.0")
        self.add_item("connection", "fetchai/http_client:0.21.0")
        self.add_item("connection", "fetchai/prometheus:0.6.0")
        self.set_config("agent.default_connection", "fetchai/p2p_libp2p:0.20.0")
        ...
        self.add_item("skill", "fetchai/advanced_data_request:0.3.0")
        self.add_item("contract", "fetchai/oracle:0.8.0")
        self.add_item("skill", "fetchai/simple_oracle:0.10.0")
        ...
        diff = self.difference_to_fetched_agent(
            "fetchai/coin_price_oracle:0.11.0", oracle_agent_name
        )
        assert (
            diff == []
        ...
        self.add_item("connection", "fetchai/ledger:0.17.0")
        self.add_item("connection", "fetchai/http_client:0.21.0")
        self.set_config("agent.default_connection", "fetchai/ledger:0.17.0")
        self.set_config("agent.default_ledger", EthereumCrypto.identifier)
        self.nested_set_config(
            "agent.required_ledgers",
            [FetchAICrypto.identifier, EthereumCrypto.identifier],
        ...
        setting_path = "agent.default_routing"
        self.nested_set_config(setting_path, default_routing)
        self.add_item("contract", "fetchai/oracle_client:0.7.0")
        self.add_item("contract", "fetchai/fet_erc20:0.7.0")
        self.add_item("skill", "fetchai/simple_oracle_client:0.8.0")
        ...
        diff = self.difference_to_fetched_agent(
            "fetchai/coin_price_oracle_client:0.8.0", client_agent_name
        )
        assert (
            diff == []
        ), "Difference between created and fetched project for files={}".format(diff)
