...
class TestWeatherSkills(AEATestCaseManyFlaky):
    ...
    def test_weather(self):
        ...
        self.add_item("skill", "fetchai/weather_station:0.25.0")
        dotted_path = (
        ...
        self.set_config("agent.default_connection", "fetchai/p2p_libp2p:0.22.0")
        self.add_item("connection", "fetchai/ledger:0.18.0")
        self.add_item("skill", "fetchai/weather_client:0.24.0")
        self.set_config("agent.default_connection", "fetchai/p2p_libp2p:0.22.0")
        dotted_path = (
...
class TestWeatherSkillsFetchaiLedger(AEATestCaseManyFlaky):
    ...
    def test_weather(self):
        ...
        diff = self.difference_to_fetched_agent(
            "fetchai/weather_station:0.28.0", weather_station_aea_name
        )
        assert (
            diff == []
        ...
        diff = self.difference_to_fetched_agent(
            "fetchai/weather_client:0.29.0", weather_client_aea_name
        )
        assert (
            diff == []
        ), "Difference between created and fetched project for files={}".format(diff)
