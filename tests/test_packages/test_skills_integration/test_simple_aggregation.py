...
class TestSimpleAggregationSkill(AEATestCaseManyFlaky, UseSOEF):
    ...
    def test_simple_aggregation(self):
        ...
        for (i, agent) in enumerate(agents):
            ...
            self.add_item("skill", "fetchai/simple_aggregation:0.3.5")
            ...
            self.set_config(setting_path, "fetchai/advanced_data_request:0.7.5")
            self.set_config(
            ...
            if i == 0:
                diff = self.difference_to_fetched_agent(
                    "fetchai/simple_aggregator:0.5.4", agent
                )
                assert (
                    diff == []
