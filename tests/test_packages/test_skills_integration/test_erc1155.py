...
class TestERCSkillsEthereumLedger(AEATestCaseManyFlaky, UseGanache):
    ...
    def test_generic(self):
        ...
        self.add_item("skill", "fetchai/erc1155_deploy:0.31.2")
        ...
        diff = self.difference_to_fetched_agent(
            "fetchai/erc1155_deployer:0.34.1", deploy_aea_name
        )
        assert (
            diff == []
        ...
        setting_path = "agent.default_routing"
        self.nested_set_config(setting_path, default_routing)
        self.add_item("skill", "fetchai/erc1155_client:0.29.2")
        ...
        diff = self.difference_to_fetched_agent(
            "fetchai/erc1155_client:0.34.1", client_aea_name
        )
        assert (
            diff == []
        ), "Difference between created and fetched project for files={}".format(diff)
