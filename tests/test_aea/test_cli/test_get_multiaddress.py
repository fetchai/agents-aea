...
class TestGetMultiAddressCommandNegativeBadHostField(AEATestCaseEmpty):
    ...
    def test_run(self, password_or_none):
        ...
        with pytest.raises(
            Exception,
            match="Host field 'some_host' not present in connection configuration fetchai/stub:0.21.2",
        ):
            self.run_cli_command(
                "get-multiaddress",
                FetchAICrypto.identifier,
...
class TestGetMultiAddressCommandNegativeBadPortField(AEATestCaseEmpty):
    ...
    def test_run(self, password_or_none):
        ...
        with pytest.raises(
            Exception,
            match="Port field 'some_port' not present in connection configuration fetchai/stub:0.21.2",
        ):
            self.run_cli_command(
                "get-multiaddress",
                FetchAICrypto.identifier,
...
class TestGetMultiAddressCommandNegativeUriNotExisting(AEATestCaseEmpty):
    ...
    def test_run(self, password_or_none):
        ...
        with pytest.raises(
            Exception,
            match="URI field 'some_uri' not present in connection configuration fetchai/stub:0.21.2",
        ):
            self.run_cli_command(
                "get-multiaddress",
                FetchAICrypto.identifier,
