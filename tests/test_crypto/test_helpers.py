...
class TestHelperFile:
    ...
    def tests_private_keys(self):
        ...
        try_validate_private_key_path(FETCHAI, FETCHAI_PRIVATE_KEY_PATH)
        with pytest.raises(SystemExit):
            private_key_path = os.path.join(
                CUR_PATH, "data", "fet_private_key_wrong.txt"
            )
            try_validate_private_key_path(FETCHAI, private_key_path)

        try_validate_private_key_path(ETHEREUM, ETHEREUM_PRIVATE_KEY_PATH)
        with pytest.raises(SystemExit):
            private_key_path = os.path.join(
                CUR_PATH, "data", "fet_private_key_wrong.txt"
            )
            try_validate_private_key_path(ETHEREUM, private_key_path)
    ...
    def test__create_ethereum_private_key_positive(self, *mocks):
        ...
        create_private_key(ETHEREUM, ETHEREUM_PRIVATE_KEY_FILE)

    @patch("builtins.open", mock_open())
    def test__create_cosmos_private_key_positive(self, *mocks):
        """Test _create_cosmos_private_key positive result."""
        create_private_key(COSMOS, COSMOS_PRIVATE_KEY_FILE)
