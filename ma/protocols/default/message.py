    ...
    def _is_consistent(self) -> bool:
        ...
        try:
            ...
            if self.performative == DefaultMessage.Performative.BYTES:
                ...
                assert type(self.content) == bytes, "Content 'content' is not of type 'bytes'."
            elif self.performative == DefaultMessage.Performative.ERROR:
                expected_nb_of_contents = 3
                assert type(self.error_code) == CustomErrorCode, "Content 'error_code' is not of type 'ErrorCode'."
                assert type(self.error_msg) == str, "Content 'error_msg' is not of type 'str'."
                assert type(self.error_data) == dict, "Content 'error_data' is not of type 'dict'."
