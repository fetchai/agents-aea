...
class PubKey:
  def __init__(self, *, strkey : Optional[str] = None, naclkey : Optional[nacl.signing.VerifyKey] = None):
    if naclkey is not None:
      self._ed25519_pub = naclkey
    elif strkey is not None:
      self._ed25519_pub =  nacl.signing.VerifyKey(strkey,
                                      encoder=nacl.encoding.HexEncoder)
    else:
      raise ValueError("Either 'strkey' or 'naclkey' must be set")

  def __str__(self):
    return self._ed25519_pub.encode(encoder=nacl.encoding.HexEncoder).decode('ascii')
...
class PrivKey:
  def __init__(self, key : Optional[str] = None):
    if key is None:
      ...
      self._ed25519 = nacl.signing.SigningKey(key,
                                      encoder=nacl.encoding.HexEncoder)
...
if __name__ == "__main__":
  ...
  print("{} keys saved to {} and {}_priv_[1-{}].txt".format(count, pubs, swarm, count))
