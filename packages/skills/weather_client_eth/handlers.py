...
if TYPE_CHECKING:
    from packages.skills.weather_client_eth.dialogues import Dialogue, Dialogues
    from packages.skills.weather_client_eth.strategy import Strategy
else:
    from weather_client_eth_skill.dialogues import Dialogue, Dialogues
...
class FIPAHandler(Handler):
    ...
    def _handle_propose(self, msg: FIPAMessage, sender: str, message_id: int, dialogue_id: int, dialogue: Dialogue) -> None:
        ...
        if proposals is not []:
            ...
            affordable = self.context.ledger_apis.token_balance('ethereum', cast(str, self.context.agent_addresses.get('ethereum'))) >= cast(int, proposal.values.get('price'))
    ...
    def _handle_match_accept(self, msg: FIPAMessage, sender: str, message_id: int, dialogue_id: int, dialogue: Dialogue) -> None:
        ...
        tx_msg = TransactionMessage(performative=TransactionMessage.Performative.PROPOSE,
                                    ...
                                    sender=self.context.agent_public_keys['ethereum'],
