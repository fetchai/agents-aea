...
@click.command(name="generate-ipfs-hashes")
@click.option("--timeout", type=float, default=15.0)
@click.option("--check", type=bool)
def generate_all_ipfs_hashes(timeout: float, check: bool) -> None:
    """Generate IPFS hashes."""
    if check:
        return_code = check_hashes(timeout)
    else:
        clean_directory()
        return_code = update_hashes(timeout)
