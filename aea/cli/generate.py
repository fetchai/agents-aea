...
def _generate_protocol(ctx: Context, protocol_specification_path: str) -> None:
    """Generate a protocol based on a specification and add it to the configuration file and agent."""
    to_local_registry = ctx.config.get("to_local_registry", False)
    ...
    try:
        protocol_generator = ProtocolGenerator(
            ...
            project_dir=(ctx.registry_path if to_local_registry else ctx.cwd),
    ...
    if os.path.exists(protocol_generator.path_to_generated_protocol_package):
        raise click.ClickException(
            "Protocol is NOT generated. The following error happened while generating the protocol:\n"
            + f"A directory with name '{protocol_spec.name}' already exists. Aborting..."
        )

    ctx.clean_paths.append(protocol_generator.path_to_generated_protocol_package)
    if to_local_registry:
        click.echo(
            f"Generating {PROTOCOL} '{protocol_spec.name}' and adding it to the the local registry"
        )
    else:
...
def _generate_full_mode(
    ...
    try:
        ...
        if to_local_registry:
            package_path = Path(protocol_generator.path_to_generated_protocol_package)
            get_package_manager(Path(ctx.registry_path)).register(
            ...
            existing_id_list.add(
                PublicId(
                    protocol_spec.author, protocol_spec.name, protocol_spec.version
                )
            )
