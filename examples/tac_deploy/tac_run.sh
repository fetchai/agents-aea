...
help() {
echo "
Use tac_run.sh <minutes_to_start> <amount_of_participatns> [contract:noncontract,fetchai] [registry:local,remote] [acn_node:local,public]

minutes_to_start - int
amount_of_participatns - int, min 2
contract - contract to use: noncontract,fetchai, ethereum. noncontract is default
registry - local or remote, local is default
acn_node - local or public, local is default
"
 exit 1;
}
...
print_options(){
start_time=$(date)
echo "Starting tac example.

Start time: ${start_time}

* minutes_to_start - ${minutes_to_start}
* amount_of_participatns - ${participants}
* contract - ${contract}
* registry - ${registry}
* acn_node - ${acn_node}
...
check_options(){
	...
	case "$contract" in
	  ...
	  ""|noncontract)
	    ...
	    ;;
	  *)
	    echo "Incorrect contract option!"
	    help
	    ;;
	esac
}
